(function () {
  ...
  function contentRefUrlsPageComputed(value) {
    if (window.flutter_inappwebview) {
      window.flutter_inappwebview.callHandler(
        "LauncherUIContentRefUrlsPageComputed",
        value
      );
    } else {
  ...
  function onTopOverlayVisibilityChanged(value) {
    console.log(
      "====== onTopOverlayVisibilityChanged, URL: " +
        window.location.href +
        ", value: " +
        value
    );
    if (window.flutter_inappwebview) {
      window.flutter_inappwebview.callHandler(
        "GestureCallbacksOnTopOverlayVisibilityChanged",
        value
      );
    } else {
      GestureCallbacksOnTopOverlayVisibilityChanged.postMessage(value);
    }
  }

  function onBottomOverlayVisibilityChanged(value) {
    console.log(
      "====== onBottomOverlayVisibilityChanged, URL: " +
        window.location.href +
        ", value: " +
        value
    );
    if (window.flutter_inappwebview) {
      window.flutter_inappwebview.callHandler(
        "GestureCallbacksOnBottomOverlayVisibilityChanged",
        value
      );
    } else {
      GestureCallbacksOnBottomOverlayVisibilityChanged.postMessage(value);
    }
  }
  ...
  xpub.navigation.openPage = function (pageRequest) {
    ...
    if (
      pageRequest.contentRefUrl !== undefined &&
      pageRequest.elementId === undefined
    ) {
      let fragmentIndex = pageRequest.contentRefUrl.indexOf("#");
      if (fragmentIndex > -1) {
        pageRequest.elementId = pageRequest.contentRefUrl.substring(
          fragmentIndex + 1
        );
      }
    }
    ...
    if (pageRequest.spineItemPageIndex !== undefined) {
      ...
      try {
        ...
        pageIndex = cfiNavigationLogic.getPageForElementCfi(
          elementCfi,
      ...
      pageRequest.spineItemPercentage > 0 &&
      paginationInfo.columnCount > 1
    ) {
      pageIndex = Math.floor(
        pageRequest.spineItemPercentage * (paginationInfo.columnCount - 1)
      );
    } else {
    ...
    if (pageIndex >= 0 && pageIndex < paginationInfo.columnCount) {
      paginationInfo.currentSpreadIndex = Math.floor(
        pageIndex / paginationInfo.visibleColumnCount
      );
      let scrollLeft =
        paginationInfo.currentSpreadIndex * xpub.paginationInfo.columnWidth;
      ...
      console.log(
        ...
        pageIndex,
        "column count is ",
        paginationInfo.columnCount
      );
    }
  };

  function getPaginationInfo(refreshAnnotations) {
    let paginationInfo = new CurrentPagesInfo(
      xpub.package,
      xpub.currentSpineItem,
      false,
      xpub.paginationInfo.pageOffset,
      xpub.paginationInfo.columnWidth
    );
    ...
    for (let i = 0, count = pageIndexes.length; i < count; i++) {
      paginationInfo.addOpenPage(
        pageIndexes[i],
        xpub.paginationInfo.columnCount,
        xpub.paginationInfo.nbThumbnailsCount,
        xpub.currentSpineItem.idref,
    ...
    paginationInfo.elementIdsWithPageIndex = mapToObj(
      xpub.elementIdsWithPageIndex
    );
  ...
  function getOpenPageIndexes() {
    ...
    let currentPage =
      _paginationInfo.currentSpreadIndex * _paginationInfo.visibleColumnCount;
    ...
    for (
      let i = 0;
      i < _paginationInfo.visibleColumnCount &&
      currentPage + i < _paginationInfo.columnCount;
      i++
    ) {
  ...
  function getVisibleContentOffsets() {
    ...
    let columnsLeftOfViewport = Math.round(
      _paginationInfo.pageOffset /
        (_paginationInfo.columnWidth + _paginationInfo.columnGap)
    );

    let topOffset = columnsLeftOfViewport * _$contentFrame.height();
    let bottomOffset =
      topOffset + _paginationInfo.visibleColumnCount * _$contentFrame.height();

    return { top: topOffset, bottom: bottomOffset };
  }

  // START Mantano (Mickaël)
  xpub.navigation.onPaginationChanged = function (
    paginationRequest_elementId,
    refreshAnnotations
  ) {
  ...
  xpub.navigation.refreshPaginationInfo = function () {
    let paginationInfo = xpub.paginationInfo;
    paginationInfo.pageOffset =
      (paginationInfo.columnWidth + paginationInfo.columnGap) *
      paginationInfo.visibleColumnCount *
      paginationInfo.currentSpreadIndex;
  ...
  xpub.navigation.toggleBookmark = function () {
    ...
    $(".xpub_page_bookmark[data-page=" + pageIndex + "] img").toggle(
      paginationInfo.pageBookmarks.length === 0
    );
  ...
  xpub.navigation.computePagesForElementId = function (
    contentRefUrls,
    sourceFileHref
  ) {
    let result = {};
    $.each(contentRefUrls, function (i, contentRefUrl) {
      let combinedPath = Helpers.ResolveContentRef(
        contentRefUrl,
        sourceFileHref
      );
      ...
      if (hashIndex >= 0) {
        ...
        result[contentRefUrl] =
          cfiNavigationLogic.getPageForElementId(elementId);
      } else {
  ...
  xpub.navigation.insureElementVisibility = function (
    spineItemId,
    element,
    initiator
  ) {
    let $element = $(element);
    if (
      cfiNavigationLogic.isElementVisible($element, getVisibleContentOffsets())
    ) {
      return;
    }
  ...
  xpub.updatePagination = function () {
    ...
    let _paginationInfo = xpub.paginationInfo;
    let isDoublePageSyntheticSpread = Helpers.deduceSyntheticSpread(
      xpub.$epubHtml,
      xpub.currentSpineItem,
      xpub.viewerSettings
    );
  ...
  xpub.openPageNext = function () {
    ...
    let lastOpenPage =
      paginationInfo.openPages[paginationInfo.openPages.length - 1];
  ...
  xpub.updateNavigationToCurrentSpreadIndex = function () {
    ...
    let scrollLeft =
      xpub.paginationInfo.currentSpreadIndex * xpubContainer[0].clientWidth;
    xpubContainer.eq(0).scrollLeft(scrollLeft);
  };
    ...
    document.fonts.ready.then(function () {
      ...
      let observerTop = new IntersectionObserver(
        function (entries) {
          // isIntersecting is true when element and viewport are overlapping
          // isIntersecting is false when element and viewport don't overlap
          //                 console.log("=========== observerTop, entry dimensions: " + entries[0].boundingClientRect.width + "x" + + entries[0].boundingClientRect.height + ", intersectionRatio: " + entries[0].intersectionRatio + ", isIntersecting? " + entries[0].isIntersecting);
          onTopOverlayVisibilityChanged(entries[0].intersectionRatio === 1.0);
        },
        { threshold: [0.0, 0.1, 0.2, 0.9, 0.95, 0.99, 1.0] }
      );
      let observerBottom = new IntersectionObserver(
        function (entries) {
          // isIntersecting is true when element and viewport are overlapping
          // isIntersecting is false when element and viewport don't overlap
          //                console.log("=========== observerBottom, entry dimensions: " + entries[0].boundingClientRect.width + "x" + + entries[0].boundingClientRect.height + ", intersectionRatio: " + entries[0].intersectionRatio + ", isIntersecting? " + entries[0].isIntersecting);
      ...
      xpub.observers.push(observerTop);
      xpub.observers.push(observerBottom);
